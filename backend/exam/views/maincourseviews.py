...
class ReadingMaterialView(APIView):
    def put(self, request, format=None):
        try:
            ...
            if course_id is None or reading_material_id is None:
                return Response({"error": "Both course_id and reading_material_id are required in the request body."},
...
class QuizModifyView(APIView):
    ...
    def patch(self, request, course_id, quiz_id, format=None):
        try:
            ...
            error_message = str(e) if isinstance(e, ObjectDoesNotExist) else "Internal Server Error"
            status_code = status.HTTP_404_NOT_FOUND if isinstance(e, ObjectDoesNotExist) else status.HTTP_500_INTERNAL_SERVER_ERROR
            return Response({"error": error_message}, status=status_code)
        
class QuestionModifyView(APIView):
    def put(self, request, course_id, quiz_id, question_id, format=None):
        try:
            # Check if course exists
            course = Course.objects.get(pk=course_id)
            if course.active:
...
# Check if quiz exists
        ...
        except (Course.DoesNotExist, Quiz.DoesNotExist, Question.DoesNotExist, ValidationError) as e:
    ...
    def patch(self, request, course_id, quiz_id, question_id, format=None):
        try:
            ...
            if isinstance(e, Question.DoesNotExist):
